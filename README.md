...
또한, 이 외부 풀업 저항 환경을 활용하여 **오픈 드레인(Open-Drain) 제어 방식**을 적용했습니다. 덕분에 MCU가 5V를 강제로 출력하고 있을 때 TM1637이 0V로 끌어내리려 하면서 발생하는 **버스 충돌(Bus Contention) 및 과전류 파손 위험**을 완벽하게 막을 수 있었습니다.

이처럼 데이터시트와 회로도를 직접 분석함으로써, 단순한 감이 아니라 하드웨어적 근거를 바탕으로 명령어 사이사이의 지연 시간(Delay)과 전압 안정화 타이밍을 설계할 수 있었습니다.

## 💡 프로젝트 후기 및 느낀 점 (Lessons Learned)

이번 프로젝트는 단순히 디스플레이에 숫자를 띄우는 '작동하는 코드'를 짜는 것을 넘어, **소프트웨어와 하드웨어의 물리적 경계가 만나는 지점**을 깊이 이해하게 된 뜻깊은 경험이었습니다. 남들이 만들어둔 편한 라이브러리를 사용했다면 5분 만에 끝났을 작업이지만, 수십 시간을 투자해 밑바닥부터 통신 프로토콜을 구현하며 다음과 같은 값진 깨달음을 얻었습니다.

**1. "이유 있는" 딜레이(Delay)를 계산하는 짜릿함**
...
**2. 데이터시트는 임베디드 개발자의 완벽한 나침반**
처음 TM1637의 타이밍 다이어그램과 D-Flip Flop 논리 회로 구조를 마주했을 때는 막막했지만, 이를 끈질기게 분석한 끝에 완벽한 동기화 타이밍을 찾아낼 수 있었습니다. 
`CLK`이 Low일 때 데이터를 안전하게 준비(Setup)하고, High로 올라가는 Rising Edge에서 데이터를 낚아채는(Latch) 과정을 직접 비트뱅잉(Bit-banging)으로 구현해 보면서, 데이터시트를 소프트웨어 로직으로 번역해 내는 능력이 크게 향상되었습니다.

**3. "소프트웨어의 실수가 하드웨어를 태울 수도 있다" (Open-Drain의 중요성)**
MCU와 TM1637이 9번째 클럭(ACK)에서 제어권을 주고받을 때, 만약 양쪽이 서로 다른 전압(5V와 0V)을 강제로 뿜어내면 쇼트(Short)가 발생해 과전류로 칩이 탈 수도 있다는 물리적 위험성을 깨달았습니다. 
이를 예방하기 위해 MCU 측에서 강제로 5V를 출력하지 않고 핀을 놓아주는 **오픈 드레인(Open-Drain) 제어 방식**을 고안 및 적용했습니다. 이를 통해 임베디드 개발자는 단순히 알고리즘을 잘 짜는 것을 넘어, 하드웨어의 전기적 안전성까지 책임지고 방어하는 코드를 작성해야 한다는 것을 배웠습니다.
